...
In~\cite{Ribeiro1996}, the causal and conflict relations for doubly-typed graph grammars over a core graph were defined. There, the graph transformation approach used was the \emph{Single Pushout} (SPO) without NACs. \cite{Corradini1996} also defined a similar notion of causal relation with respect to the DPO approach without NACs. 

Here we recall these definitions, which correspond to the \emph{unconditional causal dependency and conflict relations} defined in the following, and then extend them to create an equivalent notion of \emph{conditional relations} that work for grammars in the DPO approach with NACs.

Both cases of dependency relations defined below are based on the following intuition:
...
\begin{definition}[Unconditional Weak Conflict Relation] Given \doublyTypedGraphGrammarCore{} a doubly-typed graph grammar and \coreGraph{} a core graph. Let $a_1, a_2 \in P, a_1 \ne a_2$ and \mbox{$e_1, e_2 \in $ \coreGraph{},} $e_1 \ne e_2$. Then: 
  ...
  \begin{enumerate}
    \item The action $a_2$ is in \emph{direct weak conflict} with $a_1$, written $a_2 <_{du} a_1$, iff \mbox{$\not\exists h_{12} : L_1 \rightarrow D_2$} s.t. \mbox{$d_2 \circ h_{12} = pre_1$}.
   ...
   \item The \emph{weak conflict relation between elements} $(E, \leq_{du})$ is the reflexive and transitive closure their direct weak conflict.
  \end{enumerate}
\end{definition}

It is important to recall that the relationships extracted from a core graph whose underlying actions do not have NACs are always concrete, in the sense that if an action is dependent on (conflicting with) another one, it happens because the first one creates (resp. deletes) at least one of the elements necessary for the second to be applied.

On the other hand, dependencies and conflicts induced by NACs may be concrete, non existent, or conditioned to a specific order of application, for example: 
...
\begin{definition}[Conditional Causal Dependency Relation] Let \doublyTypedGraphGrammarCore{} be a doubly-typed graph grammar, \coreGraph{} a core graph and $P$ a set of rules with incremental, non-trivially triggered NACs only.
