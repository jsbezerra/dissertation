...
\begin{example}[Incremental NACs]Figure~\ref{fig:process:incremental-nacs} shows all possible formats that any valide incremental NAC can have.
...
\[ x =
  ...
  The first condition assures that every element in the \emph{core graph} was either already present in the initial graph or\footnote{exclusive or} was created by one and only rule. The second condition assures that for every element that is deleted, it is deleted only once by only one rule.
...
\begin{figure}[!ht]
  \centering
  \begin{subfigure}[t]{.5\textwidth}
    \centerline{\fbox{\includegraphics[scale=0.45]{images/process/unconditional-relation/core-graph}}}
...
\begin{example}[Unconditional Weak Conflict Example]
\begin{figure}[!ht]
  \centering
...
\begin{example}[Unconditional Relations]
\end{example}
...
\begin{example}[Failing Occurrence Relation with NACs]
\begin{figure}[!ht]
  \centering
  \fbox{\includegraphics[scale=0.48]{images/process/order/cycle}}
