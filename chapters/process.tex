...
order of application for any pair in that sequence and still achieve the same final graph as a result (up to isomorphism). However, this is not always true when the grammar has NACs~\cite{Corradini2013}.} This happens because when NACs come into play, there may be situations where a NAC of a rule can be triggered by the cumulative effect of applying two (or more) other rules, while the same rules would not trigger this NAC without the other. This may lead to a situation where conflicts and dependencies
