...
\newadd{In a grammar whithout NACs, if there is a sequence of graph transformations $t_0\ldots t_n$ where each pair $(t_i,t_{i+1})$ of consecutive transformations is sequentially independent, then it is possible to \emph{switch} the
...
\end{definition}
  ...
  \newadd{The restriction of the actions overlappings to the images of their matches (resp. comatch and match) considers only the local effects of these actions. Thus resulting in two properties of our interest: (1) We avoid dangling conditions with core graph edges that do not directly participate in the interaction of these actions. Similarly, (2) the NACs of these actions are not triggered with elements that, in despite of being present in the core graph, do not directly participate in
  the interaction of these two actions.}
  
  \newadd{These restrictions, together with the use of incremental NACs, will allow us to locally compute the conflicts and dependencies for each pair of actions, avoiding to deal with global effects that other actions might have over them.}

  \important{example? maybe use the the transformations of the leila's relations that were removed.}
...
\newadd{Here we recall the work of ~\cite{Corradini1996} and extend it to create an equivalent notion of \emph{occurrence relation} that works for grammars in the DPO approach with NACs.}

\begin{definition}[Causal Relation] This is the \emph{causal relation} defined in \cite{Corradini1996} for the DPO approach without NACs. Given  \doublyTypedGraphGrammarCore{} a strongly safe graph grammar, actions \mbox{$a_1, a_2 \in P, a_1 \ne a_2$} and an element \mbox{$e_1 \in N(C^T) \cup E(C^T)$}, then:
...
\newadd{\begin{thm} For each potential \emph{delete-forbid} in a strongly safe graph grammar, definition~\ref{def:delete-forbid-strong} correctly classifies this dependency.
\end{thm}}
...
\hfill
  ...
  Let $a_1,a_2 \in P$, be in a potential produce-forbid conflict according to the diagram above, where on $a_1$ creates on $H_1$ an element $x \in N(C^T) \cup E(C^T)$ which is the triggering element of a NAC $N_2$ of $a_2$ for the extended match $d_1 \circ h_{21}$. This produce-forbid conflict is:
...
\begin{description}[style=nextline,leftmargin=*]
  \item[Triggering element is related to the action:]
    Let $a_2 \leq_c x$, which means that $x$ was created by $a_2$ or by another action that must occur after $a_2$ has been applied. In such a configuration, the produce-forbid conflict is \mbox{non existent} as the element $x$ can not exist to trigger $N_2$ before the application of $a_2$.
...
\end{itemize}
  ...
  None of these conflicts or dependencies are \emph{concrete}, depending on how the orders are applied according to the causal relation and unconditional occurrence relation to exist.
