...
\begin{definition}[Abstract Conflicts and Dependencies] Given a strongly safe graph grammar \doublyTypedGraphGrammarCore{}, an abstract conflict is a tuple of distinct actions $t = (a_i,a_j,a_k)$. In any total ordering of actions in $P$, $t$ restricts the application of $a_i$ such that $a_i \not\in [a_j \ldots a_k]$.
...
\important{Conseguiste terminar isto? Para provar estes teoremas seria necessário dizer que a tua noção de OGG representa uma restrição em relação à def. de Corradini et all, e remeter para lá a questão de como transformar uma OGG em derivação e vice versa. Se as regras não envolverem NACs os teoremas do artigo do Corradini já garantem os teus. Para o 3.33 tens que argumentar que , para qualquer ordem total que satisfaça a OGG com NACs, cada acão  de P que envolve uma NAC pode ser executada
depois das ações que vem antes dela na ordem, e isto seria garantido pelas restricoes que tu impuseste ao gerar a OGG. Para o 3.35, tens que provar que a OGG construida da mesma forma como no artigo do Corradini, mas adicionando os pares referentes aos conflitos e dependencias concretos e as triplas referentes aos abstratos gera uma OGG com NACs. Ou seja, na hora de gerar a OGG com NACs, quando houver uma regra com NAC deve ser feita a analise do impacto desta regras na derivacao de acordo com
as tuas defs 3.25 e 3.27. Neste caso, precisa-se argumentar que a relacao de ocorrencia resultante é uma ordem parcial (como veio de uma ordem total, eh obvio) e que respeita as restricoes de ocorrencia (novamente, como veio de uma derivacao concreta que existe, isso deve ser obvio.}
...
\begin{definition}[OGG Derivation Format]
\end{definition}
