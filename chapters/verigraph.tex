...
The first basic type class in Verigraph is the \code{Morphism}, shown in Figure~\ref{fig:verigraph:morphism-type-class}, which serve as the minimal contract for every Category to be implemented in the tool. Notice how the contract of this type class reflects the category definition (see Definition~\ref{def:category}).
...
All the other type classes are somehow defined in terms of the morphism one. For example, the \code{Cocomplete} type class shown in Figure~\ref{fig:verigraph:cocomplete-type-class} defines some of the basic categorial constructions used in verigraph, such as coequalizers and coproducts. 

Also notice that any category that implements the functions \code{calculateCoequalizer} and \code{calculateCoproduct} automatically has a standard implementation of the \code{calculatePushout} function based only on this two constructions.

Generally speaking, whenever a category has coequalizers and coproducts, we can calculate any (finite) colimit based only on this two constructions, as demonstrated in~\cite{Pierce1991}.
