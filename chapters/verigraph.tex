...
Moreover, this code does not work only for \cat{TGraph_T}, but for any category that implements the \cat{DPO} type class. It is the case of one of the other categories implemented in verigraph, \cat{TSpan_T} but is also could be used for different categories based on graphs, algebras, logic and so on.
