...
\newadd{The retyping step is responsible to generate a doubly-typed graph grammar from the colimit calulated in the previous step. For each graph rule in $ \mbox{$p^{T} = \left(L^{T} \leftarrow K^{T} \rightarrow R^{T}\right)$} \in F_i$, we generate a new rule \mbox{$p'^{Occ_i} = \left(L^{Occ_i} \leftarrow K^{Occ_i} \rightarrow R^{Occ_i}\right)$}, where the typing morphisms are those found on the colimit. Since $Occ_i$ is now the type graph of this rules and also a typed graph over $T$, the
rules are doubly-typed over $Occ_i^T$.}
