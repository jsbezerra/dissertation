...
\newadd{In possession of the graph grammar model $GG$, the generation of test cases and oracles for a collection of functionalities $F_{\alpha}$ is accomplished by the construction of a deterministic occurrence graph grammar $OGG_{\alpha}$ by means of an amalgamation of $F_{\alpha}$ over its input-output relation $IO_{\alpha}$. The construction steps are specified in Definition~\ref{def:ogg-construction}.}
...
\item retype the rules in $F_\alpha$ over $Occ_\alpha$. In other words, use each morphism found from each $L_i, K_i, R_i$ to $Occ_\alpha$ as their respective new typing morphisms. This generates a set $F'_\alpha$ of $Occ_\alpha$ doubly-typed graph rules, since $Occ_\alpha$ is itself a $TG$ typed graph.

\item calculate the causal relation $\leq_{c}$ using the doubly-typed graph rules in $F'_\alpha$.
\item\label{enum:construction-graphs} generate the initial and final graphs $I_\alpha$ and $J_\alpha$.
...
\item\label{enum:construction-restriction} create the set $R_\alpha$ of restrictions over the ordering of rules applicability with the abstract conflicts and dependencies.
\item find (if possible) one or more total orderings respecting both the occurrence relation and the occurrence restrictions.
\end{enumerate}
...
The first step of the construction, the amalgamation of the rules in $F_i$ w.r.t. $IO_i$, is responsible to glue the graphs of all rules in one typed graph $Occ_i$, while identifying the items that are meant to be the same element at the grammar execution. Therefore, $Occ_i$ contains all elements ever to be created/preserved/deleted by any rule in $F_i$.

\newadd{The retyping step is responsible to generate a collection $F'_i$ of doubly-typed graph rules from the colimit calculated in the previous step. For each graph rule in $ \mbox{$p^{T} = \left(L^{T} \leftarrow K^{T} \rightarrow R^{T}\right)$} \in F_i$, we generate a new rule \mbox{$p'^{Occ_i} = \left(L^{Occ_i} \leftarrow K^{Occ_i} \rightarrow R^{Occ_i}\right)$}, where the typing morphisms are those from the rules graphs to the colimit graph. Since $Occ_i$ is also both the typed graph of the
new rules and a typed graph over $T$, the rules are doubly-typed over $Occ_i^T$.}

\newadd{Once having the doubly-typed graph rules, we proceed to the creation of the causal relation, as described in Definition~\ref{def:causal-relation}, which is the first indicative of whether it is possible to construct an occurrence graph grammar for this collection of rules. Remember that this relation must be a total order, otherwise it indicates a problem in the modelling or in the system.}
...
\newadd{If the steps~\ref{enum:construction-colimit} to~\ref{enum:construction-graphs} were able to be successfully performed, we have a grammar \mbox{$OGG_i = \left(Occ_i, I_i, F'_i\right)$} that is not only doubly-typed, but also strongly safe. Therefore it is a candidate to be an occurrence grammar.}
