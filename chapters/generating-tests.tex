...
\diagram{
  ...
  L_1 & K_1\ar[l]\ar[r] & R_1 & L_2 & K_2\ar[l]\ar[r] & R_2 & L_3 & K_3\ar[l]\ar[r] & R_3\\
    ...
    Figure~\ref{fig:tests:inout} shows how an $IO$ object connects two different rules, which is similar to how the interface of object of a rule identifies the elements that are the same in is left and right sides. 

    Usually, a full relation will not be necessary, as the $IO$ relation has some sort of transitivity. For example, in \tinytodo{draw and explain}
...
The colimit of a diagram such as the one presented in Example~\ref{ex:inout} has the following format, where $Occ$ is the amalgamation object, i.e. it contains all the concrete elements of the rules where it came from:
...
If the result of this colimit is a \emph{core graph} according to definition~\ref{def:core-graph}, we can create a doubly-typed graph grammar $OGG_i$ that is also a strongly-safe grammar and therefore a candidate to be an occurrence graph grammar. Otherwise, there is more than one rule that deletes or creates
the same element, thus it is not possible to apply all rules of this functionality.


\begin{example}[Amalgamation example]\label{ex:amalgamation} Figure~\ref{fig:tests:colimit} shows the object resultant from the amalgamation of the rules in Figure~\ref{fig:tests:grammar}.
