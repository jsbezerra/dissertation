...
\newadd{Given the graph grammar model of a system, we want to generate test cases and test oracles for a collection of (integrated) functionalities. For example, it could be something as ``atomic'' as a successful or failing login or something as complex as the entire workflow of attending a client, which could require attendants and waiters to be logged in the system, tables to be occupied, orders to be prepared, among others.}

\newadd{\begin{definition}[Functionality Collection] Given a graph grammar \graphGrammar{} modelling a system, a \emph{functionality collection} $F$ is a collection of the grammar rules. The term collection here is used to imply that the same rule $r \in P$ can appear more than one time in $F$, which in this context means the functionality modelled by $r$ is intended to be executed more than once.
\end{definition}}

\begin{example} Figure~\ref{fig:tests:grammar} shows such a functionality collection from the restaurant grammar. The chosen rules represent one way in which the entire process of serving a client can be accomplished. In this particular case, we chose the rules that represent the use cases \emph{reserve table}, \emph{accommodate client}, \emph{serve table}, \emph{close table}. \important{explain the goals of each functionality before showing the rules}. \important{show one more example where it fails?}
...
\begin{definition}[Input-Output Relation] \newadd{Given a functionality collection $F_i$, its underlying \emph{input-output relation} $IO_i$ is a set of typed-graph morphism spans of the form \mbox{$\left(R_x \leftarrow IO \rightarrow L_y\right)$} each of which connects two distinct rules $x,y \in F_i$.}
  ...
  \newadd{An input-output relation for a functionality collection will have an appearance similar to the diagram bellow, where there are several $IO$ objects connecting the right-hand side of a rule with the left-hand side of another.}
...
\begin{example}[Input-Output Relations]\label{ex:inout} \important{review} \important{Figure~\ref{fig:tests:inout} shows how an $IO$ object connects two different rules, which is similar to how the interface graph of a rule identifies the elements that are the same in its left and right sides.}\important{put all IO objects for the example}
...
\newadd{Having the graph grammar \graphGrammar{} model of a system with \mbox{$n > 0$} functionalities, the generation of test cases and oracles for each functionality $i$ is accomplished by the construction of a deterministic occurrence graph grammar $OGG_i$ using $F_i$ and $IO_i$. The construction steps are specified in Definition~\ref{def:ogg-construction}.}

\begin{definition}[Deterministic Occurrence Graph Grammar Construction]\label{def:ogg-construction} Let \graphGrammar{} be a graph grammar, $F_i$ be a functionality collection and $IO_i$ its corresponding input-output relation.
...
\item retype the rules in $F_i$ over $Occ_i$, generating a set $F'_i$ of doubly-typed graph rules.
