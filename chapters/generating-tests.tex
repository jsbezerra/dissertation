...
\important{explain tests and oracles}

Basically, we are interested in knowing whether each functionality of a system is truly executable \important{leila: why?}, i.e. whether all the rules that represent each functionality flow are applicable.
...
Given a graph grammar \graphGrammar{} that models a system, we want to generate test cases and oracles for each subset of rules $F_i \subseteq P$ $\forall i \in 1\ldots n$, where $F_i$ represents a \uline{complete functionality of the system}. \important{define}
