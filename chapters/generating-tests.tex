...
\newadd{Having a graph grammar \graphGrammar{} modelling a system with \mbox{$n > 0$} functionalities, the generation of the test cases and oracles for each functionality $i$ is done by the construction of a deterministic occurrence graph grammar $OGG_i$ using $F_i$ and $IO_i$. The construction steps are specified in Definition~\ref{def:ogg-construction}.}.
...
\begin{enumerate}
\item calculate the amalgamation (colimit) $Occ_i$ of the rules in $F_i$ with respect to $IO_i$ and retype the rules $F_i$, generating a set $F'_i$ of doubly-typed graph rules.
\item calculate the occurrence relation for the rules in $F'_i$.
...
\end{enumerate}
  ...
  The Occurrence Graph Grammar representing the functionality $i$ is given by $OGG_i = (Occ_i, I_i,F_i)$, iff $OGG_i$ respects the conditions imposed by Definition~\ref{def:ogg}
\end{definition}
...
Remember that, if there is more than one rule that deletes or creates the same element, it is not possible to apply all rules of this functionality, and $OGG_i$ can not be a deterministic occurrence graph grammar. \tinytodo{put retyped rules?}
