...
\newadd{If the steps~\ref{enum:construction-colimit} to~\ref{enum:construction-graphs} were able to be performed, than we have a grammar \mbox{$OGG_i = \left(Occ_i, I_i, F'_i\right)$} that is not only doubly-typed, but also strongly safe. Therefore it is a candidate to be an occurrence grammar.}
