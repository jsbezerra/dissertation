...
\begin{definition}[Pushout] Given 

\diagram{
  A\ar[r]\ar[d] & B\ar[d]\ar[rdd] &\\
  C\ar[r]\ar[drr]       & X\ar[dr]&\\
                &         &X'
}
...
\begin{definition}[Pullback] \tinytodo{Maintain only if we are going to use the concurrent rules chapter} Given two objects $A$ and $B$
\end{definition}
