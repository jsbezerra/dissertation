...
\begin{definition}[Graph Constraint] A (typed) graph constraint is a \emph{boolean} formula over atomic (typed) graph constraints, such that:
  \begin{itemize}
    \item $true$ is graph constraint
    \item every atomic constraint is also a graph constraint
    \item if $c$ is a graph constraint, then $\neg c$ is also a graph constraint
    \item for every graph constraint $c_i$ with $i \in I$ for some index set $I$, both $\land_{i \in I} c_i$ and $\lor_{i \in I} c_i$ are also graph constraints
    \item nothing else is a graph constraint
  \end{itemize}

  A graph $G$ satisfies a graph constraint $c$ 
\end{definition}
