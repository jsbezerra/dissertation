...
Category theory is also the basis for graph transformation systems, which is central to the work proposed in this thesis. Therefore, we present a brief introduction of the field and the basic categorial constructions that will be necessary in this work.
...
\begin{example}[Category Examples] \tinytodo{Use set examples and cite some others}
\end{example}
...
\begin{example}[Mail Server Graph Grammar] In the following, we describe graph grammar that models a client-server scenario for a very simple e-mail application. This system has only four actions: 
...
The rules for this grammar are depicted in Fig~\ref{fig:gts:mail}\tinytodo{Draw type graph}. 
