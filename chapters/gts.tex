...
We now review the basic concepts and some analysis techniques for graph transformations that are used in this work. We use the algebraic approach, which is based on Category Theory. For the proofs of theorems and facts, the reader is reffered to ~\cite{Corradini1996:DPO}, however a brief review of the main categorial concepts used in this thesis is presented in appendix~\ref{app:category-theory}.
...
In this work we mostly consider dependencies (and conflicts) between rules. Thus, when we say that two rules are causally dependent it means that there may be a situation in which the application of one rule actually depends on the application of the other, but there might be also situations in which these both rules are applied independently.
