...
\begin{definition}[Coproduct] Given two objects $A$ and $B$, their \emph{coproduct} (also called \emph{categorical sum}) is an object $A+B$ and two injection arrows \morph{i_A}{A}{A+B} and \morph{i_B}{B}{A+B} such that, for any other object $C$ an pair of arrows \morph{f}{A}{C} and \morph{g}{B}{C} there is one unique arrow \morph{!}{A+B}{C} such that the following diagram commutes:
...
\begin{example}[Coproducts in \cat{Set}] Coproducts can be used to generalize the notion of disjoint union. Figue~\ref{fig:gts:coproduct} shows an example of a coproduct in \cat{Set}.
...
\end{figure}
  ...
  Having the sets $A = \{1,2,3\}$ and $B = \{1,2\}$ as objects, we have that the set $A+B$ together with morphisms $i_A,i_B$ is a coproduct because the morphisms are total (all elements of $A$ and $B$ are mapped to $A+B$) and no element is identified of the sets is identified in $A+B$.

  In this example, $C$ can not be a coproduct because we can not find a unique arrow \morph{!}{C}{A+B}: there are multiple choices due to the different possible functions mapping $A$ and $B$ to $C$. Also, an object such as $D = \{(1,A),(2,AB),(3,A),(1,B)\}$ can not be a coproduct because is not possible to find an arrow such that the diagram commutes: all the possible functions from $A$ and $B$ would identify some elements of the sources.

  Notice that $(A+B)' = \{(1,0),(2,0),(3,0),(1,1),(2,1)\}$ or $(A+B)'' = \{a,b,c,d,e\}$ would also be equally valid as coproducts. This is due to the fact the categories deal with their objects up to isomorphism, i.e. all this objects have the same format regardless of their internal representations. Therefore, they can be considered equivalent.\tinytodo{rephrase.}
\end{example}
...
\begin{example}[Coequalizers in \cat{Set}] Coequalizers generalize the notion of smallest equivalence relation. In \cat{Set} \tinytodo{draw examples}.
...
\begin{example}[Pushouts in \cat{Set}] In \cat{Set} \tinytodo{draw example}
...
\begin{example}[Colimits in \cat{Set}] Colimits generalize several constructions such as disjoint unions, direct sums, coproducts, pushouts and others, where different objects of a diagram are ``glued'' together in only one object respecting existent identifications, if any.

  In \cat{Set} \tinytodo{draw example}
\end{example}
...
\begin{example}[Mail Server Grammar] Fig~\ref{fig:gts:mail} \tinytodo{Draw type graph} depicts a graph grammar that models a client-server scenario for a very simple e-mail application. This system has only four actions: 
...
\begin{figure}[!ht]
  ...
  \end{subfigure}
  \caption{Mail application graph grammar}\label{fig:gts:mail}
\end{figure}
