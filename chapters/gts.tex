...
\begin{definition}[Coproduct] Given two objects $A$ and $B$, their \emph{coproduct} (also called \emph{categorical sum}) is an object $A+B$ and two injection arrows \morph{i_1}{A}{A+B} and \morph{i_2}{B}{A+B} such that, for any other object $C$ an pair of arrows \morph{f}{A}{C} and \morph{g}{B}{C} there is one unique arrow \morph{!}{A+B}{C} such that the following diagram commutes:
...
\begin{example}[Coproducts in \cat{Set}] Coproducts can be used to generalize the notion of disjoint union. Particularly, in \cat{Set} we have:\tinytodo{draw example}
...
\begin{example}[Mail Server] Fig~\ref{fig:gts:mail} \tinytodo{Draw type graph} depicts a graph transformation system that models a client-server scenario for a very simple e-mail application. This system has only four actions: 
